...
router.post('/options', isLoggedInADMIN , async (req,res) => {
...
router.get('/rebaixar/:id',isLoggedInADMIN, async (req,res) => {
    await mysql.query('UPDATE galeria SET privilegio = 0 WHERE idImagens ="'+req.params.id+'" ')
    res.redirect('/admin/galeria')
})

router.get('/elevar/:id',isLoggedInADMIN, async (req,res) => {
...
router.get('/aproveitar/:id',isLoggedInADMIN ,async (req,res) => {
